...
\chapter{语步和语阶辨认}\label{chapter2}

\section{语步辨认}

医学RA的摘要是由一些语步组成的，这些语步共同实现其交流的目的。在这个意义上，语步可定义为 “文本中执行特定交流功能的部分” (Kanoksilapatham, 2007, p.23)。

大多数医学论文摘要都采用了四步走的方案，第一步（M1）确定研究领域，第二步（M2）描述研究过程，第三步（M3）总结主要成果，第四步（M4）得出结论。这些都是医学研究报告摘要中的常规语步。

在结构化的摘要中，M2、M3和M4可以通过方法(method)、结果(result)和结论(conclusion)的标题轻松识别。虽然有些摘要中没有“方法”的具体标题，但M2被细分为几个语阶，可以清楚地识别。它们可以被标记为 “实验设计(design)”、“实验环境设置(setting)”、“参与者(participants)”、“干预(interventions)”、“主要观察指标(main outcome measures)”，等等。

当涉及到M1时，标有“目标(objective)”或“背景(background)”或两者皆有的章节都属于M1部分的内容。在一些结构化的摘要中，“背景” 和“目标”都被标示出来，有些只有“目标”，而有些只有“背景”。那些标有“目标”或“背景”的部分，在实际内容上通常由这两个部分组成。此外，在实践过程中，目标和背景密切相关，通常被视为一个整体，以实现确定研究领域这一特定交流目的。
...
\chapter{语步四}\label{chapter6}

语步4（M4）是医学学术论文摘要的最后一步，在这里可以强调和解释关键性的结果，指出研究的意义并给出建议。也可以对未来的工作或后续研究提出建议。在以假设为导向的研究中，也需要在这一语步中明确说明假设是否得到了结果的支持。
